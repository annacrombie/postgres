...
{
	pgstat_schedule_stat_drop(PGSTAT_KIND_DB, databaseid, InvalidOid);
}
 ...
 */
void
pgstat_database_reset_timestamp(Oid dboid, TimestampTz ts)
{
	PgStatSharedRef *dbref;
	PgStatShm_StatDBEntry *dbentry;

	dbref = pgstat_shared_stat_locked(PGSTAT_KIND_DB, MyDatabaseId,
									  InvalidOid, false);

	dbentry = (PgStatShm_StatDBEntry *) dbref->shared_stats;
	dbentry->stats.stat_reset_timestamp = ts;

	pgstat_shared_stat_unlock(dbref);
}

/*
 * pgstat_database_flush_cb - flush out a local database stats entry
 *
 ...
 */
bool
pgstat_database_flush_cb(PgStatSharedRef *shared_ref, bool nowait)
{
	PgStatShm_StatDBEntry *sharedent;
...
void
pgstat_database_reset_timestamp_cb(PgStatShm_StatEntryHeader *header, TimestampTz ts)
{
	((PgStatShm_StatDBEntry *) header)->stats.stat_reset_timestamp = ts;
}
