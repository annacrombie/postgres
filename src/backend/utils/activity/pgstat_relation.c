...
pgstat_copy_relation_stats(Relation dst, Relation src)
{
	 ...
	 * indexes via stats?
	 */
	dst_ref = pgstat_shared_stat_locked(PGSTAT_KIND_TABLE,
...
{
	pgstat_schedule_stat_create(PGSTAT_KIND_TABLE,
...
pgstat_drop_relation(Relation rel)
{
	...
	pgstat_schedule_stat_drop(PGSTAT_KIND_TABLE,
							  rel->rd_rel->relisshared ? InvalidOid : MyDatabaseId,
							  RelationGetRelid(rel));
...
pgstat_report_autovac(Oid dboid)
{
	 ...
	 */
	shared_ref = pgstat_shared_stat_locked(PGSTAT_KIND_DB, dboid, InvalidOid,
										   false);
...
/*
 * pgstat_relation_flush_cb - flush out a pending rlation stats entry
 *
 ...
 */
bool
pgstat_relation_flush_cb(PgStatSharedRef *shared_ref, bool nowait)
{
...
void
pgstat_relation_pending_delete_cb(PgStatSharedRef *shared_ref)
{
