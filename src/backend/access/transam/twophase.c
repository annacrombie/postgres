...
StartPrepare(GlobalTransaction gxact)
{
	...
	hdr.ncommitstats =
		pgstat_pending_stats_drops(true, &commitstats);
	hdr.nabortstats =
		pgstat_pending_stats_drops(false, &abortstats);
...
FinishPreparedTransaction(const char *gid, bool isCommit)
{
	...
	if (isCommit)
		pgstat_perform_drops(hdr->ncommitstats, commitstats, false);
	else
		pgstat_perform_drops(hdr->nabortstats, abortstats, false);
