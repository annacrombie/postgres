...
extern PGDLLIMPORT void llvm_enter_fatal_on_oom(void);
extern PGDLLIMPORT void llvm_leave_fatal_on_oom(void);
extern PGDLLIMPORT bool llvm_in_fatal_on_oom(void);
extern PGDLLIMPORT void llvm_reset_after_error(void);
extern PGDLLIMPORT void llvm_assert_in_fatal_section(void);

extern PGDLLIMPORT LLVMJitContext *llvm_create_context(int jitFlags);
extern PGDLLIMPORT LLVMModuleRef llvm_mutable_module(LLVMJitContext *context);
extern PGDLLIMPORT char *llvm_expand_funcname(LLVMJitContext *context, const char *basename);
extern PGDLLIMPORT void *llvm_get_function(LLVMJitContext *context, const char *funcname);
extern PGDLLIMPORT void llvm_split_symbol_name(const char *name, char **modname, char **funcname);
extern PGDLLIMPORT LLVMTypeRef llvm_pg_var_type(const char *varname);
extern PGDLLIMPORT LLVMTypeRef llvm_pg_var_func_type(const char *varname);
extern PGDLLIMPORT LLVMValueRef llvm_pg_func(LLVMModuleRef mod, const char *funcname);
extern PGDLLIMPORT void llvm_copy_attributes(LLVMValueRef from, LLVMValueRef to);
extern PGDLLIMPORT LLVMValueRef llvm_function_reference(LLVMJitContext *context,
						LLVMBuilderRef builder,
						LLVMModuleRef mod,
						FunctionCallInfo fcinfo);

extern PGDLLIMPORT void llvm_inline(LLVMModuleRef mod);
 ...
 ****************************************************************************
 */
extern PGDLLIMPORT bool llvm_compile_expr(struct ExprState *state);
struct TupleTableSlotOps;
extern PGDLLIMPORT LLVMValueRef slot_compile_deform(struct LLVMJitContext *context, TupleDesc desc,
...
/** Get the host CPU as a string. The result needs to be disposed with
  LLVMDisposeMessage. */
extern PGDLLIMPORT char *LLVMGetHostCPUName(void);
#endif
...
extern PGDLLIMPORT char *LLVMGetHostCPUFeatures(void);
#endif

extern PGDLLIMPORT unsigned LLVMGetAttributeCountAtIndexPG(LLVMValueRef F, uint32 Idx);
