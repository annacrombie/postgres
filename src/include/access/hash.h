...
extern PGDLLIMPORT IndexBuildResult *hashbuild(Relation heap, Relation index,
								   struct IndexInfo *indexInfo);
extern PGDLLIMPORT void hashbuildempty(Relation index);
extern PGDLLIMPORT bool hashinsert(Relation rel, Datum *values, bool *isnull,
					   ItemPointer ht_ctid, Relation heapRel,
					   IndexUniqueCheck checkUnique,
					   bool indexUnchanged,
					   struct IndexInfo *indexInfo);
extern PGDLLIMPORT bool hashgettuple(IndexScanDesc scan, ScanDirection dir);
extern PGDLLIMPORT int64 hashgetbitmap(IndexScanDesc scan, TIDBitmap *tbm);
extern PGDLLIMPORT IndexScanDesc hashbeginscan(Relation rel, int nkeys, int norderbys);
extern PGDLLIMPORT void hashrescan(IndexScanDesc scan, ScanKey scankey, int nscankeys,
					   ScanKey orderbys, int norderbys);
extern PGDLLIMPORT void hashendscan(IndexScanDesc scan);
extern PGDLLIMPORT IndexBulkDeleteResult *hashbulkdelete(IndexVacuumInfo *info,
											 IndexBulkDeleteResult *stats,
											 IndexBulkDeleteCallback callback,
											 void *callback_state);
extern PGDLLIMPORT IndexBulkDeleteResult *hashvacuumcleanup(IndexVacuumInfo *info,
												IndexBulkDeleteResult *stats);
extern PGDLLIMPORT bytea *hashoptions(Datum reloptions, bool validate);
extern PGDLLIMPORT bool hashvalidate(Oid opclassoid);
extern PGDLLIMPORT void hashadjustmembers(Oid opfamilyoid,
							  Oid opclassoid,
							  List *operators,
							  List *functions);
...
/* hashinsert.c */
extern PGDLLIMPORT void _hash_doinsert(Relation rel, IndexTuple itup, Relation heapRel);
extern PGDLLIMPORT OffsetNumber _hash_pgaddtup(Relation rel, Buffer buf,
								   Size itemsize, IndexTuple itup);
extern PGDLLIMPORT void _hash_pgaddmultitup(Relation rel, Buffer buf, IndexTuple *itups,
								OffsetNumber *itup_offsets, uint16 nitups);

/* hashovfl.c */
extern PGDLLIMPORT Buffer _hash_addovflpage(Relation rel, Buffer metabuf, Buffer buf, bool retain_pin);
extern PGDLLIMPORT BlockNumber _hash_freeovflpage(Relation rel, Buffer bucketbuf, Buffer ovflbuf,
									  Buffer wbuf, IndexTuple *itups, OffsetNumber *itup_offsets,
									  Size *tups_size, uint16 nitups, BufferAccessStrategy bstrategy);
extern PGDLLIMPORT void _hash_initbitmapbuffer(Buffer buf, uint16 bmsize, bool initpage);
extern PGDLLIMPORT void _hash_squeezebucket(Relation rel,
								Bucket bucket, BlockNumber bucket_blkno,
								Buffer bucket_buf,
								BufferAccessStrategy bstrategy);
extern PGDLLIMPORT uint32 _hash_ovflblkno_to_bitno(HashMetaPage metap, BlockNumber ovflblkno);

/* hashpage.c */
extern PGDLLIMPORT Buffer _hash_getbuf(Relation rel, BlockNumber blkno,
						   int access, int flags);
extern PGDLLIMPORT Buffer _hash_getbuf_with_condlock_cleanup(Relation rel,
												 BlockNumber blkno, int flags);
extern PGDLLIMPORT HashMetaPage _hash_getcachedmetap(Relation rel, Buffer *metabuf,
										 bool force_refresh);
extern PGDLLIMPORT Buffer _hash_getbucketbuf_from_hashkey(Relation rel, uint32 hashkey,
											  int access,
											  HashMetaPage *cachedmetap);
extern PGDLLIMPORT Buffer _hash_getinitbuf(Relation rel, BlockNumber blkno);
extern PGDLLIMPORT void _hash_initbuf(Buffer buf, uint32 max_bucket, uint32 num_bucket,
						  uint32 flag, bool initpage);
extern PGDLLIMPORT Buffer _hash_getnewbuf(Relation rel, BlockNumber blkno,
							  ForkNumber forkNum);
extern PGDLLIMPORT Buffer _hash_getbuf_with_strategy(Relation rel, BlockNumber blkno,
										 int access, int flags,
										 BufferAccessStrategy bstrategy);
extern PGDLLIMPORT void _hash_relbuf(Relation rel, Buffer buf);
extern PGDLLIMPORT void _hash_dropbuf(Relation rel, Buffer buf);
extern PGDLLIMPORT void _hash_dropscanbuf(Relation rel, HashScanOpaque so);
extern PGDLLIMPORT uint32 _hash_init(Relation rel, double num_tuples,
						 ForkNumber forkNum);
extern PGDLLIMPORT void _hash_init_metabuffer(Buffer buf, double num_tuples,
								  RegProcedure procid, uint16 ffactor, bool initpage);
extern PGDLLIMPORT void _hash_pageinit(Page page, Size size);
extern PGDLLIMPORT void _hash_expandtable(Relation rel, Buffer metabuf);
extern PGDLLIMPORT void _hash_finish_split(Relation rel, Buffer metabuf, Buffer obuf,
							   Bucket obucket, uint32 maxbucket, uint32 highmask,
							   uint32 lowmask);

/* hashsearch.c */
extern PGDLLIMPORT bool _hash_next(IndexScanDesc scan, ScanDirection dir);
extern PGDLLIMPORT bool _hash_first(IndexScanDesc scan, ScanDirection dir);

/* hashsort.c */
typedef struct HSpool HSpool;	/* opaque struct in hashsort.c */

extern PGDLLIMPORT HSpool *_h_spoolinit(Relation heap, Relation index, uint32 num_buckets);
extern PGDLLIMPORT void _h_spooldestroy(HSpool *hspool);
extern PGDLLIMPORT void _h_spool(HSpool *hspool, ItemPointer self,
					 Datum *values, bool *isnull);
extern PGDLLIMPORT void _h_indexbuild(HSpool *hspool, Relation heapRel);

/* hashutil.c */
extern PGDLLIMPORT bool _hash_checkqual(IndexScanDesc scan, IndexTuple itup);
extern PGDLLIMPORT uint32 _hash_datum2hashkey(Relation rel, Datum key);
extern PGDLLIMPORT uint32 _hash_datum2hashkey_type(Relation rel, Datum key, Oid keytype);
extern PGDLLIMPORT Bucket _hash_hashkey2bucket(uint32 hashkey, uint32 maxbucket,
								   uint32 highmask, uint32 lowmask);
extern PGDLLIMPORT uint32 _hash_spareindex(uint32 num_bucket);
extern PGDLLIMPORT uint32 _hash_get_totalbuckets(uint32 splitpoint_phase);
extern PGDLLIMPORT void _hash_checkpage(Relation rel, Buffer buf, int flags);
extern PGDLLIMPORT uint32 _hash_get_indextuple_hashkey(IndexTuple itup);
extern PGDLLIMPORT bool _hash_convert_tuple(Relation index,
								Datum *user_values, bool *user_isnull,
								Datum *index_values, bool *index_isnull);
extern PGDLLIMPORT OffsetNumber _hash_binsearch(Page page, uint32 hash_value);
extern PGDLLIMPORT OffsetNumber _hash_binsearch_last(Page page, uint32 hash_value);
extern PGDLLIMPORT BlockNumber _hash_get_oldblock_from_newbucket(Relation rel, Bucket new_bucket);
extern PGDLLIMPORT BlockNumber _hash_get_newblock_from_oldbucket(Relation rel, Bucket old_bucket);
extern PGDLLIMPORT Bucket _hash_get_newbucket_from_oldbucket(Relation rel, Bucket old_bucket,
												 uint32 lowmask, uint32 maxbucket);
extern PGDLLIMPORT void _hash_kill_items(IndexScanDesc scan);

/* hash.c */
extern PGDLLIMPORT void hashbucketcleanup(Relation rel, Bucket cur_bucket,
